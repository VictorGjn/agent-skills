...
def main() -> int:
    ...
    # Loose denominator = tasks with ground truth in ANY run (after the
    # n_truth>0 filter render_aggregate_table applies internally). Compute
    # explicitly so the both-views narrative cites a real number.
    all_with_gt = {tid for run in runs for r in run
                    if (tid := r.get("task_id")) is not None
                    and r.get("metrics", {}).get("n_truth", 0) > 0}
    ...
    if args.both_views:
        md.append(f"_All-tasks-with-GT view (n={len(all_with_gt)} union); reachable-only view (n={len(reachable)} intersection of non-error task IDs across all {len(runs)} runs)._")
        md.append("")
        md.append(f"## Aggregate metrics — all tasks with GT (n={len(all_with_gt)}, loose)")
        md.append("")
    ...
    # Footer: regen command (mirrors what was actually invoked) so future-self
    # doesn't have to grep history to refresh a generated report.
    md.append("")
    md.append("---")
    md.append("")
    md.append("_Regenerate this file:_")
    md.append("")
    # Normalize Windows backslashes to forward slashes so the regen command
    # is shell-portable (the file may be regenerated on Linux/Mac CI).
    def _portable(p) -> str:
        return str(p).replace("\\", "/")

    md.append("```bash")
    md.append("python eval/csb/diff_runs.py \\")
    md.append(f"  --runs {' '.join(_portable(p) for p in args.runs)} \\")
    md.append(f"  --names {' '.join(args.names)} \\")
    if args.both_views:
        md.append("  --both-views \\")
    elif args.reachable_only:
        md.append("  --reachable-only \\")
    md.append(f"  --top-n {args.top_n} \\")
    md.append(f"  --output {_portable(args.output)}")
    md.append("```")
    md.append("")
