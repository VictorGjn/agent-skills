...
**Known limitation (M11 multi-source freshness)**: The `sources` field (array of source type strings, e.g. `["web", "code"]`) is part of the wiki-doc-layer specification (M11 markdown frontmatter) and is used by `compute_freshness_multi_source()` to apply per-source half-lives (web=30d, code=90d, etc.). However, real corpus entities sourced from system-of-record integrations (HubSpot, back-office) do NOT carry a `sources` field — they only have `last_verified_at`. As a result, `compute_freshness_multi_source()` is unreachable in production today; all entities fall back to the generic `compute_freshness()` with a default 60-day half-life. This limitation will be resolved when wiki-doc-layer entities (with `sources` populated from markdown frontmatter) enter the corpus. `sources` was removed from the golden-corpus fixtures in this PR (schema v7 does not carry it — see entity.schema.json), so `compute_freshness_multi_source()` is currently exercised by neither production nor the test suite; it ships ahead of its callers pending the wiki-doc-layer schema migration.
